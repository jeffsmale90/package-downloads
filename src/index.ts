...
async function getStats() {
  ...
  if (
    ...
    (process.argv.length === 3 && process.argv[2].endsWith("index.ts"))
  ) {
    program.help();
...
async function runStats(
  ...
  switch (granularity) {
    case "daily":
      startDateApi = formatDateForApi(
        calculateStartDate(today, numPoints, "days")
      );
      apiRange = `${startDateApi}:${endDateApi}`;
      break;
    case "weekly":
      startDateApi = formatDateForApi(
        calculateStartDate(today, numPoints * 7, "days")
      ); // Fetch daily for aggregation
      apiRange = `${startDateApi}:${endDateApi}`;
      break;
    case "monthly":
      startDateApi = formatDateForApi(
        calculateStartDate(today, numPoints * 30, "days")
      ); // Fetch daily for aggregation
      apiRange = `${startDateApi}:${endDateApi}`;
      break;
    default:
  ...
  try {
    ...
    const response = await fetch(
      `https://api.npmjs.org/downloads/range/${apiRange}/${packageName}`
    );
    if (!response.ok) {
    ...
    if (data.downloads && data.downloads.length > 0) {
      ...
      if (granularity === "daily") {
        processedDownloads = data.downloads.map(
          (entry: { day: string; downloads: number }) => entry.downloads
        );
        processedPeriods = data.downloads.map(
          (entry: { day: string; downloads: number }) => entry.day
        );
      ...
      if (processedDownloads.length === 0) {
        console.log(
          `No download data available for the selected period and granularity (${granularity}).`
        );
        process.exit(0);
      }
      ...
      for (const line of chartLines) {
        ...
        effectiveChartDataWidth = Math.max(
          effectiveChartDataWidth,
          lastCharIndex + 1
        );
      }
      ...
      xAxisLabels[processedPeriods.length - 1] = formatPeriodLabel(
        processedPeriods[processedPeriods.length - 1],
        granularity
      );
      ...
      for (let i = interval; i < processedPeriods.length - 1; i += interval) {
        if (i > 0 && i < processedPeriods.length - 1) {
          // Ensure not to overwrite start/end
      ...
      Object.keys(xAxisLabels).forEach((indexStr) => {
        ...
        let position = Math.floor(
          (index / (processedPeriods.length - 1)) *
            (effectiveChartDataWidth - 1)
        );
        ...
        if (
          position >= 0 &&
          position + label.length <= effectiveChartDataWidth
        ) {
          let canPlace = true;
...
function aggregateToWeeks(
  ...
  dailyDownloads.forEach((entry) => {
    ...
    const weekKey = `${startOfWeek.getFullYear()}-${(startOfWeek.getMonth() + 1)
      .toString()
  ...
  return Object.keys(weeklyData)
    .sort()
    .map((key) => ({ period: key, downloads: weeklyData[key] }));
}
