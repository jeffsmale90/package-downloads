...
function getStats() {
    return __awaiter(this, void 0, void 0, function* () {
        const program = new commander_1.Command('package-downloads');
        program
...
function runStats(packageName, granularity, numPoints) {
    return __awaiter(this, void 0, void 0, function* () {
        var _a;
        ...
        const calculateStartDate = (baseDate, points, type) => {
        ...
        try {
            ...
            const metadataResponse = yield fetch(`https://registry.npmjs.org/${packageName}`);
            if (!metadataResponse.ok) {
                const errorText = yield metadataResponse.text();
                ...
                console.log(`Latest Version: ${((_a = metadata['dist-tags']) === null || _a === void 0 ? void 0 : _a.latest) || 'N/A'}`);
                console.log(`Homepage: ${metadata.homepage || 'N/A'}`);
                console.log('\n' + '-'.repeat(80) + '\n'); // Separator
            }
            const response = yield fetch(`https://api.npmjs.org/downloads/range/${apiRange}/${packageName}`);
            if (!response.ok) {
            ...
            if (data.downloads && data.downloads.length > 0) {
                ...
                if (granularity === 'daily') {
                    processedDownloads = data.downloads.map((entry) => entry.downloads);
                    processedPeriods = data.downloads.map((entry) => entry.day);
                }
                ...
                const formatPeriodLabel = (periodString, type) => {
...
function aggregateToWeeks(dailyDownloads) {
    const weeklyData = {};
...
function aggregateToMonths(dailyDownloads) {
    const monthlyData = {};
    dailyDownloads.forEach(entry => {
        const date = new Date(entry.day);
